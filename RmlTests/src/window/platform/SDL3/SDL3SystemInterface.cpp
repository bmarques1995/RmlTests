...
RmlTests::SDL3SystemInterface::SDL3SystemInterface(std::shared_ptr<Window> window) :
	m_Window(window)
{
}

RmlTests::SDL3SystemInterface::~SDL3SystemInterface()
{
	m_Window = nullptr;
}

double RmlTests::SDL3SystemInterface::GetElapsedTime()
{
...
void RmlTests::SDL3SystemInterface::SetMouseCursor(const Rml::String& cursor_name)
{    
    Rml::String cursorOutputName = cursor_name.empty() ? "arrow" : cursor_name;
    if (Rml::StringUtilities::StartsWith(cursor_name, "rmlui-scroll"))
        cursorOutputName = "move";

	m_Window->ModifyCursor(cursorOutputName);
}

void RmlTests::SDL3SystemInterface::SetClipboardText(const Rml::String& text)
{
    SDL_SetClipboardText(text.c_str());
}

void RmlTests::SDL3SystemInterface::GetClipboardText(Rml::String& text)
{
...
void RmlTests::SDL3SystemInterface::ActivateKeyboard(Rml::Vector2f caret_position, float line_height)
{
    const SDL_Rect rect = { int(caret_position.x), int(caret_position.y), 1, int(line_height) };
    SDL_SetTextInputArea(std::any_cast<SDL_Window*>(m_Window->GetWindowPointer()), &rect, 0);
    SDL_StartTextInput(std::any_cast<SDL_Window*>(m_Window->GetWindowPointer()));
}

void RmlTests::SDL3SystemInterface::DeactivateKeyboard()
{
    SDL_StopTextInput(std::any_cast<SDL_Window*>(m_Window->GetWindowPointer()));
}

bool RmlTests::InputEventHandler(Rml::Context* context, SDL_Window* window, SDL_Event& ev)
{
	...
	#pragma region RMLSDL_WINDOW_EVENTS_BEGIN
...
Rml::Input::KeyIdentifier RmlTests::ConvertKey(int sdlkey)
{
...
int RmlTests::ConvertMouseButton(int button)
{
	switch (button)
	{
...
int RmlTests::GetKeyModifierState()
{
...
Rml::TouchList RmlTests::TouchEventToTouchList(SDL_Event& ev, Rml::Context* context, SDL_FingerID finger_id)
{
